...
Active Work
    ...
    ✅ COMPLETE: System cleanup (5 orphaned folders deleted)

    ✅ COMPLETE: Path corrections in .ai-context.md v3.3 → v3.4

    ✅ COMPLETE: Git repos synced and pushed

    ✅ COMPLETE: Space Instructions updated (single SOT)

    ✅ COMPLETE: Private repo established as SOT for .ai-context.md

    ⏳ NEXT: Resume v3.1.2 bugfix work (TEXT WALL PDF, DryRun UX)
...
Blockers
    ...
    ✅ RESOLVED: Space Instructions simplified

    ✅ RESOLVED: .ai-context.md SOT established (private repo)

Development Roadmap

    v3.1.1 (CURRENT): ✅ Stable production engine

    v3.1.2 (NEXT): 🔧 Bugfixes (TEXT WALL PDF, DryRun UX)

    v3.2.0 (PLANNED): ⚡ Performance (40-80× speedup via Distil-Whisper, parallel processing)

    v3.3.0 (FUTURE): 🤖 Automation (scheduled episode checks, batch missing episodes)

FILE SYNC WORKFLOW
Sync Order (NEVER REVERSE)

    Private Repo First: Commit to D:\Desktop\SecurityNow-Full-Private\

    Then Public: Run Special-Sync.ps1 to copy tools/docs → public repo

    Validation: Confirm NO media files in public repo
...
❌ Mistake #4: GRC HTML Parsing - Wrong Regex
...
powershell
# ✅ CORRECT - Uses HTML entity &#160; (non-breaking space)
$episodePattern = 'Episode&#160;(\d{1,4})&#160;-'

# ❌ WRONG - These fail on GRC HTML
...
❌ Mistake #5: PowerShell Syntax - goto Labels
...
powershell
# ❌ WRONG - This is batch/cmd syntax
:Done
goto Done
...
Every new development thread MUST:
    ...
    Read this file (.ai-context.md) from private repo

    Read NEW-THREAD-CHECKLIST.md for session workflow

    Read COMMON-MISTAKES.md for error prevention patterns

    Verify paths with Test-Path before proceeding
...
When path confusion occurs:
    ...
    ⚠️ STOP - Check this file FIRST

    Verify: Test-Path "D:\Desktop\SecurityNow-Full-Private\"

    Do NOT proceed with wrong paths
...
Thread Policy: Treat older threads as retired sources.
               Don't re-summarized in new threads.
...
✅ AUDIT CORRECTIONS APPLIED - COMPREHENSIVE ERROR PREVENTION
✅ PRIVATE REPO AS SOT - SYNCED TO PUBLIC FOR SPACE ACCESS