...
### Fixed
- **TEXT WALL Bug**: Fixed poor PDF readability in AI transcripts by adding optimized wkhtmltopdf rendering flags
  - Added `--dpi 200` for higher resolution (was default 96 DPI)
  - Added `--print-media-type` for better print CSS layout
  - Added `--no-pdf-compression` for 15% faster conversion
  - Added proper margins (15mm top/bottom, 12mm left/right)
  - Trade-off: ~30% larger file sizes for dramatically improved text clarity

- **DryRun UX Confusion**: Fixed misleading "Failed" messages in dry-run mode
  - Added `AiSkipped` counter to distinguish intentional skips from actual failures
  - Summary now shows "Skipped (dry-run): X" in cyan instead of "Failed: X" in red
  - Eliminates user confusion about script health during testing

### Changed
- Updated version to 3.1.2 with release date 2026-01-16
...
- Archived previous versions to `archive/historical-dev`
- Created git tags for version tracking

### Fixed
- Verified Whisper paths (`C:\tools\whispercpp`)
- Confirmed GRC regex pattern with non-breaking space character (&nbsp; / &#160;)
...
### Added
- Integration with `episode-dates.csv` for accurate year sorting
...
- Episode date caching system (episode-dates.csv)
